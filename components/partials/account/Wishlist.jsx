...
const Wishlist = ({ ecomerce }) => {
    ...
    function handleAddItemToCart(e, product) {
        e.preventDefault();
        addItem({ id: product.id, quantity: 1, shopId:product.shop.id }, ecomerce.cartItems, 'cart');
    }
                    ...
                    <tbody>
                        {products.map((product) => (
                            <tr key={product.id}>
                                ...
                                <td className="price">${product.discount?product.price - product.discount: product.price}</td>
