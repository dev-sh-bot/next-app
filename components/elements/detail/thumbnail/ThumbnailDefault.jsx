...
import { baseUrl, baseUrlImageNew } from '~/repositories/Repository';
...
const ThumbnailDefault = ({ product, vertical = true }) => {
    ...
    useEffect(() => {
        ...
        if (product && product.image.length > 0) {
            product.image.map((item) => {
                images.push(`${baseUrlImageNew}${item.image}`);
            });
    ...
    if (productImages.length > 0) {
        ...
        galleryImagesView = productImages.map((item, index) => (
            <div className="item" key={item}>
                <a href="#" onClick={(e) => handleOpenLightbox(e, index)} className="d-flex justify-content-center">
                    {/* <img src={item} alt={item} /> */}
                    <img src={item} alt={item} style={{ maxHeight: "400px", objectFit: "contain" }} />
                </a>
            </div>
        ));
    }
