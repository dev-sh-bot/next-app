...
const RelatedProduct = ({ slug , collectionSlug, boxed, layout }) => {
    const [productItems, setProductItems] = useState([]);
    const [loading, setLoading] = useState(true);

    async function getRelatedProducts() {
        setLoading(true);
        const responseData = await ProductRepository.getProductsByCategory(slug, "category", "retailer");
        if (responseData) {
            setProductItems(responseData.Product);
            setTimeout(
                function () {
                    setLoading(false);
                }.bind(this),
